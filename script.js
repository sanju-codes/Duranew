...
(function(){
  ...
  function buildProductViewer(){
    ...
    var LEVITATE_Y = 0.55; // rest height above the pad — the "antigravity" float
    productViewer.levitateY = LEVITATE_Y;
    sheetGroup.position.y = LEVITATE_Y;
    scene.add(sheetGroup);
    productViewer.sheetGroup = sheetGroup;

    // Soft ground shadow "pad" the sheet hovers above — breathes with the bob
    var padTex = (function(){
    ...
    shadowPad.position.y = -1.5;
    scene.add(shadowPad);
    productViewer.shadowPad = shadowPad;
    ...
    function animate(){
      ...
      var group = productViewer.sheetGroup;
      group.rotation.y = rotY;
      group.rotation.x = rotX;

      // Antigravity float: gentle sine bob, shadow pad breathes opposite it
      if(!REDUCED_MOTION){
        var t = bobClock.getElapsedTime();
        var bob = Math.sin(t * 0.9) * 0.16;
        group.position.y = LEVITATE_Y + bob;
        var lift = (bob + 0.16) / 0.32; // 0..1, 1 = highest
        productViewer.shadowPad.material.opacity = 0.75 - lift * 0.35;
        var s = 1 - lift * 0.18;
        productViewer.shadowPad.scale.set(s, s, 1);
      } else {
        group.position.y = LEVITATE_Y;
      }
  ...
  function swapProduct(idx){
    ...
    group.position.y = productViewer.levitateY || 0;
